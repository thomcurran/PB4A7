---
title: 'AT4: Hypothesis Testing and Causal Inference'
output:
  html_document:
    df_print: paged
---
...
Now we are going to open the file in R using RStudio. Use the command below, but be sure to change the filepath to the one where you have put the WV6_Data_R_v20180912.rds file. Alternatively, find the WV6_Data_R_v20180912.rds file in the AT4 workshop folder, click on it, and save it as WVS.
...
## Two-parameter model or independent t-test with confidence intervals
...
```{r}
WVSimmigrant$V39 <- revalue(as.character(WVSimmigrant$V39), c("2"="0")) # revalue is part of the plyr package and takes the V39 variable from the WVSimmigrant data frame and replaces values of 2 with values of 0. This function always takes the input of a factor or categorical variable so that as.character code just forces V39 to a category (i.e., not a numeric variable).
```
...
We might write this in a research paper like so:
...
WVSfinance %>% 
  ...
  xlab("Satisfaction with finances") + # add appropriate x lab
...
```{r eval=FALSE}
finance.model <- lm(V23 ~ 1 + V59, data=WVSfinance)
...
summary(finance.boot) ## add the finance boot model and stipulate how many resamples you want to run (typically 5,000)
```
