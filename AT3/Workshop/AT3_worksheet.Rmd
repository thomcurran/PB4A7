...
4. Set up a three-parameter linear model to test a multiple relationship
...
Before we begin analysing, there are missing values in the GDP and gini variables, so they will need to be dealt with. Lets filter the GDP and gini variables to get rid of the cases with missing values.
...
As we saw in the lecture, one of the great things about the linear model is that it can take any number of predictor variables and use them to explain one outcome variable. It is often the case in psychology that we would be interested in studying multiple predictor variables, rather than just one. For one thing, the addition of predictor variables allows us to control for other third variable influences on relationships (i.e., does x predict y controlling for z?) For another thing, the addition of predictor variables allows us to ascertain the relative importance of variables on an outcome (- for example, w)i.e., which is the best predictor of y, is it x or is it z?)

There are other reasons why we would add variables to our linear models, not least to improve the accuracy of our predictions or explain more variance in the outcome. When we add more than one continuous predictor variable to our linear model we are conducting multiple regression.

So lets now employ multiple regression on our WHR dataset and add the Gini coefficient to the simple regression model we just tested. The gini coefficient is a metric of economic inequality and some theories propose that economic inequality can be damaging for happiness. The greater the gaps between the rich and poor, the less happy people are with their lives. 

We have just seen that country wealth (i.e., GDP) has a strong positive correlation with happiness but can we improve this model, or explain more variance, by adding Gini to it? 
...
You know by now that the first step in any test of relationships is to inspect the extent to which our variables co-vary. However, this is especially important for multiple regression because the correlations between the predictor variables are removed from the linear model to yield unique effects of the predictor on the outcome (more below). Lets therefore compute the correlation matrix for our variables using the rcorr() function.
...
Okay, lets now give you a chance to set up and test a linear regression model. We have just seen that GDP has a strong positive simple relationship with happiness but what about inequality, what is the nature of its simple relationship with happiness? Lets find out. 
...
## Task 1 - Plot the distribution of the GINI coefficient
...
## Task 2 - Calcualte the correlation coefficient for the relationship between GINI and happiness
...
```{r eval=FALSE}
rcorr(as.matrix(WHR[,c("??","??")], type="pearson")) # fill in the ?? gaps here for the explanatory variable and the correlation type (pearson)
```
...
## Task 4 - Setting up the linear model
...
## Task 5 - The standardized estimate
