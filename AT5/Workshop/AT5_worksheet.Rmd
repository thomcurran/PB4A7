---
title: 'AT5: Mediation and Moderation'
output:
  html_document:
    df_print: paged
---
...
```{r eval = FALSE}
install.packages("https://github.com/thomcurran/PB130/blob/master/Packages/probemod_0.2.tar.gz?raw=true", repos = NULL, type = "source")
...
First, lets load this data into our R environment. Go to the AT7 folder, and then to the workshop folder, and find the "employee.csv" file. Click on it and then select "import dataset". In the new window that appears, click "update" and then when the dataframe shows, click import. If you want, you can try running the code below replace my file loacation for yours and it should do the same thing (if not put your hand up).
...
```
We can also call all this information at once using the new interactions package.

```{r}
install.packages("pequod")
library(pequod)
jn_output <- johnson_neyman(mod.model, alpha = 0.05)
...
Another way to do this is via the interactions package. It gives a more colourful version of the same output.

```{r}
install.packages("interactions")
library(interactions)
johnson_neyman(mod.model, pred = "Structure", modx = "Control",)
```
