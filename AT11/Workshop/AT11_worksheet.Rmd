---
title: "AT11: Multilevel Modeling"
...
As we saw in AT1, when we run tests using the GLM we make several assumptions. Some, like normality of residuals and linearity, we can test for. But some, like perfect measurement and independence, we cannot.
...
As always, let's first load in the school data. You can add it manually from the file pane or replace my file location with yours in the code below:
...
As always, let's first load in the stress data. You can add it manually from the file pane or replace my file location for yours in the code below:
...
confint(time.stress.fit2, method="boot", nsim=100, oldNames=FALSE)
```
